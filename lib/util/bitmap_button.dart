...
class BitmapButton extends PositionComponent
    with HasPaint, TapCallbacks, KeyboardHandler {
  //
  ...
  BitmapButton({
    ...
    if (size == null) {
      ...
      this.size.x =
          (this.size.x ~/ cornerSize * cornerSize).toDouble() + cornerSize * 2;
      this.size.y =
          (this.size.y ~/ cornerSize * cornerSize).toDouble() + cornerSize * 2;
    } else {
      this.size = size;
    }
